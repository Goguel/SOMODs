...
public class NovoValorProdController {
	...
	@FXML
	private TextField txtNovoValorProd;
	@FXML
	private TextField txtBuscaProd;
	@FXML
	private Button btnAplicar;
	
	public NovoValorProdController() {
		...
		alert3.setContentText("Valor alterado com suscesso");
	}
	
	public void onAplicarClick() throws IOException {
		if(txtBuscaProd.getText() != "") {
			if(txtNovoValorProd.getText() != "") {
				if(conP.buscaProduto(Integer.parseInt(txtBuscaProd.getText()))){
				conP.alteraProduto(Integer.parseInt(txtBuscaProd.getText()), null, 2, Float.parseFloat(txtNovoValorProd.getText()), 0);
				alert3.show();
